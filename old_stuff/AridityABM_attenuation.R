...
alf <- function(f, h, FrO, FrN, Temp, Temp0) {
  alpha <- 869*(f^2)*(((1.84e-11)*((Temp/Temp0)^(0.5))) + ((Temp/Temp0)^(-5/2)) * ( 0.01275*(exp(-2239.1/Temp)/((FrO^2+f^2)/FrO)) + 0.1068*(exp(-3352/Temp)/((FrN^2+f^2)/FrN)) ) ) 
  return(alpha)
}
...
oxy <- function(f, h, FrO, FrN, Temp, Temp0) {
  ox <- 869*(f^2)*(((Temp/Temp0)^(-5/2)) * ( 0.01275*(exp(-2239.1/Temp)/((FrO^2+(f^2))/FrO)) ) ) 
  return(ox)
}

nit <- function(f, h, FrO, FrN, Temp, Temp0) {
  ni <- 869*(f^2)*(((Temp/Temp0)^(-5/2)) * ( 0.1068*(exp(-3352/Temp)/((FrN^2+(f^2))/FrN)) ) ) 
  return(ni)
}
