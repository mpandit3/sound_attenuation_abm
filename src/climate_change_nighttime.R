...
for(i in 1:length(years)){ #beginning of weather data loop
  ...
  stid <- "ERIC"
  stations <- read.csv("C:/Users/meely/OneDrive - University of Oklahoma/University of Oklahoma/Ross Lab/Aridity and Song Attenuation/Aridity Agent Based Model/abm_month_timeframe_090120/geoinfo.csv", 
                       stringsAsFactors = F)
  ...
  ###Use night_temp function to calculate higher nighttime temperatures for future climate change conditions
 # source("~/OneDrive - University of Oklahoma/University of Oklahoma/Ross Lab/Aridity and Song Attenuation/Aridity Agent Based Model/abm_month_timeframe_052420/src/night_temp.R")
...
wdsum = wdsum_total %>% #Finds Means of data 
  # rename(TAIR = fTAIR) %>%
  filter(!is.na(bin1))
  
cc_final = NULL
for(i in 1:length(wdsum$TAIR)){
  if(between(wdsum$bin1[i],105,895)){
    cc_temp = wdsum$TAIR[i]-2.64
  } else if(between(wdsum$bin1[i],0,105)){
    cc_temp = wdsum$TAIR[i]+12.11
  } else if(between(wdsum$bin1[i],900,1435)){
    cc_temp = wdsum$TAIR[i]+12.11
  }
...
# out = cbind(wdsum_total$bin1,wdsum_total$TAIR,cc_final)
wdsum$TAIR = cc_final

#Calculating Evaporative Waterloss
wdsum = wdsum %>%
  filter(!is.na(TAIR)) %>%
  filter(!is.na(bin1))

wdsum$tewl = 0 #total evaporative water loss increases until it reaches 4.9
wdsum$ewl = NULL
for(j in 1:length(wdsum$bin1)){
  
  wdsum$ewl[j] = (0.0045*exp((0.1511*(wdsum$TAIR[j]))))/12 #ewl equation divided by 12 to account for the 5 min bins, using linear equation from HOFI since they are similar mass
  # wd$tewl = wd$tewl[j]+wd$ewl[j]
  if(j == 1){
    wdsum$tewl[j] = 0 + wdsum$ewl[j]
  } else {
    wdsum$tewl[j] = wdsum$tewl[j-1]+wdsum$ewl[j]
  }
  if(wdsum$bin1[j] == 0| wdsum$bin1[j] == 5){
    wdsum$tewl[j] = 0
  } else{
...
setwd("C:/Users/meely/OneDrive - University of Oklahoma/University of Oklahoma/Ross Lab/Aridity and Song Attenuation/Aridity Agent Based Model/abm_month_timeframe_090120/data/ERIC_weather_cc_nighttime")
fname = paste0(stid, "_weather_cc_nighttime")
